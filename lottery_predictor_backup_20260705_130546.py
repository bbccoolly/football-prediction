# lottery_predictor.py — 彩票号码智能预测引擎 v3
"""
基于历史数据的自学习预测系统。
特性:
  1. 时间衰减加权频率（近期数据权重更高）
  2. 6种策略并行：热号追踪、冷号反弹、温冷回补、遗漏回补、模式匹配、综合加权
  3. 回测验证：留出最近N期测试各策略命中率
  4. 自适应选策：自动选用回测胜率最高的策略
  5. 模式分析：奇偶比、大小比、和值区间、连号检测
  6. 后区独立分析（大乐透）

v3 改进（基于 2026-07-05 回测学习）:
  - HALF_LIFE 12→20：更平衡的冷热权重
  - 新增温冷回补策略：针对遗漏5-15期的"甜蜜点"区间
  - DLT权重调整：综合加权优先，温冷回补次之
  - 降低综合加权随机性：70%确定性核心+30%随机扰动
  - 后区独立冷热搭配分析
"""
...
# ============================================================
# 策略权重（基于 2026-07-05 回测学习修正）
# 每次开奖后可更新此配置
# ============================================================
LEARNED_WEIGHTS = {
    "dlt":  {"weighted": 5, "warm_cold": 4, "hot": 3, "pattern": 2, "cold": 1, "missing": 1},
...
def _strategy_warm_cold(decayed: dict, lottery_key: str, params: dict, history: list = None) -> list:
    """策略3：温冷回补 — 选择遗漏5-15期的'甜蜜点'号码（v3新增）
    研究发现：遗漏<5期(过热)和>20期(过冷)反弹概率低，5-15期是最佳反弹区间。
    本期验证：前区25(遗漏8期)、后区01(遗漏10期)均在此区间。
    """
    if history is None:
        history = []
    if "zones" in params:
        ...
        for zi, zone in enumerate(params["zones"]):
            ...
            # 筛选甜蜜点区间（5-15期） + 结合频率信息
            freq_data = decayed.get(zone["name"], {})
            sweet_spot = []
            for n in all_nums:
                miss = last_seen.get(n, len(history))
                if 5 <= miss <= 15:
                    # 综合评分 = 频率权重 × (1 + 遗漏/10) — 遗漏越久但不太久，权重越高
                    freq_w = freq_data.get(n, 0.001)
                    score = freq_w * (1.0 + miss / 10.0)
                    sweet_spot.append((n, score, miss))

            sweet_spot.sort(key=lambda x: -x[1])

            if sweet_spot:
                # 从甜蜜点中选 zone["count"]*3 个候选，然后随机选
                candidates = [n for n, _, _ in sweet_spot[:zone["count"] * 3]]
                if len(candidates) >= zone["count"]:
                    selected = random.sample(candidates, zone["count"])
                else:
                    # 甜蜜点不足，用综合加权补足
                    selected = list(candidates)
                    freqs = decayed.get(zone["name"], {})
                    remaining = sorted(freqs.items(), key=lambda x: -x[1])
                    for n, _ in remaining:
                        if n not in selected and len(selected) < zone["count"]:
                            selected.append(n)
                picks.extend(sorted(selected, key=lambda x: int(x)))
            else:
                # 无甜蜜点，退化为综合加权
                picks.extend(_strategy_weighted_single_zone(zone, decayed))
        return picks
            ...
            for i, draw in enumerate(history):
                ...
                if nums and p < len(nums):
                    ...
                    if periods < last_seen.get(n, 999):
                        last_seen[n] = periods
            sweet = [(n, m) for n, m in last_seen.items() if 5 <= m <= 15]
            if sweet:
                sweet.sort(key=lambda x: -x[1])
                candidates = [n for n, _ in sweet[:5]]
...
def _strategy_weighted_single_zone(zone: dict, decayed: dict) -> list:
    """综合加权单区辅助函数"""
    zn = zone["name"]
    freqs = decayed.get(zn, {})
...
def _strategy_weighted(decayed: dict, lottery_key: str, params: dict, history: list = None) -> list:
    """策略4：综合加权 — 确定性核心+随机扰动（v3优化：降低70%随机性）
    先按频率选出前70%的确定性号码，再在剩余中随机选取。
    """
    if "zones" in params:
        picks = []
        for zi, zone in enumerate(params["zones"]):
            ...
            # v3改进：确定性核心 + 随机扰动
            # 选出前 zone["count"]*2 个高频号，其中确定70%
            top_pool = [n for n, _ in items[:zone["count"] * 4]]
            if len(top_pool) < zone["count"]:
                top_pool = [n for n, _ in items]

            # 确定性选择：取top_pool中频率最高的ceil(count*0.7)个
            import math as _math
            deterministic_count = _math.ceil(zone["count"] * 0.7)
            deterministic = top_pool[:deterministic_count]

            # 随机扰动：从剩余中随机选
            remaining_pool = [n for n in top_pool if n not in deterministic]
            random_count = zone["count"] - len(deterministic)
            if random_count > 0 and remaining_pool:
                selected_random = random.sample(
                    remaining_pool,
                    min(random_count, len(remaining_pool))
                )
            else:
                selected_random = []

            selected = deterministic[:deterministic_count] + selected_random
            selected = selected[:zone["count"]]
            picks.extend(sorted(selected, key=lambda x: int(x)))
        return picks
