...
各 PR 从前一个已验证提交创建连续分支，不在一个 PR 中混合全部重构：
...
> 当前状态：共享服务、标准 ID 请求、分域模型、特征协议、产物与权重门禁、不可变快照、原子刷新及 Web/CLI 迁移均已实现，并已通过自动化测试、启动日志和桌面/移动端浏览器验收。XGBoost、神经网络、Stacking、可信 Walk-forward 和正式模型准入仍按范围留在 PR 4。
